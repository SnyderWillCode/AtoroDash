...
const fetchAnnouncements = async () => {
  try {
    ...
    announcements.value = data.map((item: { id: number; title: string; shortDescription: string; description: string; assets: { images: string[] }[]; date: string; tags: { tag: string }[] }) => ({
      id: item.id,
      title: item.title,
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between mb-6">
      ...
      <button @click="scrollToEnd" class="text-[#a855f7] hover:text-[#9333ea] transition-colors duration-200 text-sm">
      ...
      <div ref="scrollContainer" class="flex space-x-6 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide">
        <div v-for="announcement in announcements" :key="announcement.id"
          ...
          <div class="p-6 space-y-4">
            ...
            <button @click="openAnnouncement(announcement)"
  ...
  <Teleport to="body">
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click="closeModal">
      ...
      <div
        ...
        :class="{ 'opacity-0 scale-95': !isModalMounted, 'opacity-100 scale-100': isModalMounted }" @click.stop>
          ...
          <div v-if="selectedAnnouncement" class="relative">
            ...
            <div class="relative aspect-video">
              <div class="flex transition-transform duration-300 ease-out"
                ...
                <img v-for="(image, index) in selectedAnnouncement.images" :key="index" :src="image"
                  :alt="`${selectedAnnouncement.title} - Image ${index + 1}`"
                  class="w-full h-full object-cover flex-shrink-0" />
              </div>

              <!-- Image navigation -->
              <div v-if="selectedAnnouncement.images.length > 1"
                class="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2">
                <button v-for="(_, index) in selectedAnnouncement.images" :key="index"
                  @click="currentImageIndex = index" class="w-2 h-2 rounded-full transition-colors duration-200"
                  :class="index === currentImageIndex ? 'bg-white' : 'bg-white/50'">
                </button>
              </div>

              <!-- Arrow navigation -->
              <button v-if="selectedAnnouncement.images.length > 1 && currentImageIndex > 0"
              ...
              <button
                v-if="selectedAnnouncement.images.length > 1 && currentImageIndex < selectedAnnouncement.images.length - 1"
                @click="currentImageIndex++"
            ...
            <div class="p-8 space-y-6">
              <div class="space-y-4">
                ...
                <!-- Tags -->
                <div class="flex flex-wrap gap-2">
                  <span v-for="tag in selectedAnnouncement.tags" :key="tag"
